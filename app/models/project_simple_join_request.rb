class ProjectSimpleJoinRequest < ActiveRecord::Base
  unloadable
  belongs_to :user
  belongs_to :project
  ...
  named_scope :visible_to, lambda {|user|
    {
      :include => :project,
      :conditions => Project.allowed_to_condition(user, :approve_project_simple_join_requests)
    }
  }
  ...
  def decline!
    ...
    ProjectSimpleJoinRequestMailer.deliver_declined_request(self)
    self
  end
  ...
  def self.create_request(user, project)
    ProjectSimpleJoinRequest.create(:user => user, :project => project, :status => 'new')
  end

  def self.pending_request_for?(user, project)
    ProjectSimpleJoinRequest.find_by_user_id_and_project_id(user.id, project.id)
  end
end
